...
export function getRepositoryToken(
  ...
  if (
    ...
    if (!dataSourcePrefix && !namespacePrefix) {
      return entity;
    }
    return `${namespacePrefix}${dataSourcePrefix}${getCustomRepositoryToken(entity)}`;
  }

  if (entity instanceof EntitySchema) {
    return `${namespacePrefix}${dataSourcePrefix}${
      entity.options.target ? entity.options.target.name : entity.options.name
    }Repository`;
  }
  return `${namespacePrefix}${dataSourcePrefix}${entity.name}Repository`;
}
